  ...
  describe("getChunderURI", () => {
    ...
    describe("without edge and with region", () => {
      describe("for deprecated known regions", () => {
        Object.entries(deprecatedRegions).forEach(
          ([deprecatedRegion, preferredRegion]) => {
            describe(deprecatedRegion, () => {
              it("should call the deprecation handler and recommend an edge", async () => {
                ...
                await new Promise((resolve) =>
                  setTimeout(() => {
                    ...
                    assert(
                      onDeprecated.args[0][0].match(
                        new RegExp(`please use \`edge\` "${preferredEdge}"`)
                      )
                    );
                    resolve(undefined);
                  })
              ...
              it("should return the right chunder uri", () => {
                const uris = getChunderURIs(
                  undefined,
                  deprecatedRegion,
                  onDeprecated
                );
                assert.deepEqual(uris, [
                  `chunderw-vpc-gll-${preferredRegion}.twilio.com`,
                ]);
              });
            });
          }
        ...
        it("should call the deprecation handler, but not recommend an edge", async () => {
          ...
          await new Promise((resolve) =>
            setTimeout(() => {
              assert(onDeprecated.calledOnce);
              assert.equal(
                onDeprecated.args[0][0].match(new RegExp("edge", "g")).length,
                2
              );
              assert.equal(
                onDeprecated.args[0][0].match(new RegExp("please use", "g")),
                null
              );
              resolve(undefined);
            })
        ...
        it("should call the deprecation handler and recommend an edge", async () => {
          ...
          await new Promise((resolve) =>
            setTimeout(() => {
              ...
              assert.equal(
                onDeprecated.args[0][0].match(
                  new RegExp(`please use \`edge\` "roaming"`, "g")
                ).length,
                1
              );
              resolve(undefined);
    ...
    describe("with edge and without region", () => {
      describe("for known edges", () => {
        Object.entries(edgeToRegion)
          .filter(([e]) => e !== defaultEdge)
          .forEach(([edge, region]) => {
            describe(edge, () => {
              it("should not call the deprecation handler", async () => {
