...
describe("Connection", function () {
  ...
  const MediaHandler = () => {
    mediaStream = createEmitterStub(
      require("../../lib/twilio/rtc/peerconnection")
    );
    mediaStream.setInputTracksFromStream = sinon.spy((rejectCode?: number) => {
      return rejectCode
        ? Promise.reject({ code: rejectCode })
        : Promise.resolve();
    });
    mediaStream.answerIncomingCall = sinon.spy(
      (a: any, b: any, c: any, d: RTCConfiguration, cb: Function) => {
        callback = cb;
        rtcConfig = d;
      }
    );
    ...
    mediaStream.mute = sinon.spy((shouldMute: boolean) => {
      mediaStream.isMuted = shouldMute;
    });
    mediaStream.version = {
      pc: {},
      getSDP: () =>
  ...
  const StatsMonitor: any = () =>
    (monitor = createEmitterStub(
      require("../../lib/twilio/statsMonitor").default
    ));
  ...
  describe("constructor", () => {
    ...
    context("when outgoing", () => {
      it("should send outgoing event to insights", () => {
        conn = new Connection(
          config,
          Object.assign(options, { preflight: true })
        );
  ...
  describe(".accept", () => {
    ...
    context("when getInputStream is not present", () => {
      it("should call mediaStream.openWithConstraints with audioConstraints if passed", () => {
        ...
        sinon.assert.calledWith(mediaStream.openWithConstraints, {
          foo: "bar",
        });
      });
    ...
    context("when getInputStream is present and succeeds", () => {
      ...
      context("when incoming", () => {
        ...
        it("should call mediaStream.answerIncomingCall with override `rtcConfiguration`", () => {
          ...
          return wait.then(() => {
            assert.deepEqual(
              mediaStream.answerIncomingCall.args[0][3],
              rtcConfiguration
            );
          });
        });
      ...
      context("when outgoing", () => {
        ...
        beforeEach(() => {
          ...
          mediaStream.makeOutgoingCall = sinon.spy(
            (a: any, b: any, c: any, d: any, e: any, _callback: Function) => {
              callback = _callback;
            }
          );
        });
        ...
        it("should call mediaStream.makeOutgoingCall with correctly encoded params", () => {
          ...
          return wait.then(() => {
            ...
            assert.equal(
              mediaStream.makeOutgoingCall.args[0][1],
        ...
        it("should call mediaStream.makeOutgoingCall with an override `rtcConfiguration`", () => {
          ...
          return wait.then(() => {
            assert.deepEqual(
              mediaStream.makeOutgoingCall.args[0][4],
              rtcConfiguration
            );
          });
        });
  ...
  describe(".disconnect()", () => {
    [
      ...
      context(`when state is ${state}`, () => {
        ...
        ["answer", "cancel", "hangup", "ringing", "transportClose"].forEach(
          (eventName: string) => {
    ...
    [Connection.State.Pending, Connection.State.Closed].forEach(
    ...
    [
      ...
      context(`when connection state is ${state}`, () => {
        ...
        it("should not emit cancel", () => {
          conn.on("cancel", () => {
            throw new Error("Should not have emitted cancel");
          });
          conn.ignore();
        });
      ...
      [true, undefined].forEach((value?: boolean) => {
        context(`when ${value}`, () => {
          ...
          it("should not emit mute", () => {
            conn.on("mute", () => {
              throw new Error("Expected mute to not be emitted");
            });
            conn.mute(value);
          });
        });
    ...
    [
      ...
      context(`when connection state is ${state}`, () => {
        ...
        it("should not emit reject", () => {
          conn.on("reject", () => {
            throw new Error("Should not have emitted reject");
          });
          conn.reject();
        });
  ...
  describe(".postFeedback()", () => {
    ...
    Object.values(Connection.FeedbackScore).forEach(
      (score: Connection.FeedbackScore) => {
        Object.values(Connection.FeedbackIssue).forEach(
    ...
    it("should throw if score is invalid", () => {
      assert.throws(() => {
        (conn as any)["postFeedback"](0);
      });
    });

    it("should throw if issue is invalid", () => {
      assert.throws(() => {
        (conn as any)["postFeedback"](Connection.FeedbackScore.Five, "foo");
      });
    });
  });
  ...
  describe(".sendDigits()", () => {
    context("when digit string is invalid", () => {
      ["foo", "87309870934z", "09-823-", "ABC", "9 9"].forEach(
        (digits: string) => {
    ...
    context("when digit string is valid", () => {
      ["12345", "1w2w3w", "*#*#*#", "1#*w23####0099"].forEach(
        (digits: string) => {
  ...
  context("in response to", () => {
    ...
    describe("mediaStream.onerror", () => {
      const baseError = {
        info: { code: 123, message: "foo", twilioError: "bar" },
      };
        ...
        [
          ...
          context(`and state is ${state}`, () => {
            ...
            it("should call pstream.hangup with error message", () => {
              mediaStream.onerror(
                Object.assign({ disconnect: true }, baseError)
              );
              sinon.assert.calledWith(
                pstream.hangup,
    ...
    describe("mediaStream.onopen", () => {
      ...
      [Connection.State.Ringing, Connection.State.Connecting].forEach(
        (state: Connection.State) => {
          context(`when state is ${state}`, () => {
            beforeEach(() => {
              (conn as any)["_status"] = state;
            ...
            context("when this connection is not answered", () => {
              ...
              it("should not emit Connection.accept event", () => {
                conn.on("accept", () => {
                  throw new Error("Expected to not emit accept event");
                });
                mediaStream.onopen();
                clock.tick(1);
              });
    ...
    describe("mediaStream.onclose", () => {
      ...
      it("should play the disconnect ringtone if shouldPlayDisconnect is not specified", () => {
        ...
        sinon.assert.calledOnce(
          soundcache.get(Device.SoundName.Disconnect).play
        );
      });
      ...
      it("should play the disconnect ringtone if shouldPlayDisconnect returns true", () => {
        conn = new Connection(
          config,
          Object.assign({ shouldPlayDisconnect: () => true }, options)
        );
        mediaStream.onclose();
        sinon.assert.calledOnce(
          soundcache.get(Device.SoundName.Disconnect).play
      ...
      it("should not play the disconnect ringtone if shouldPlayDisconnect returns false", () => {
        conn = new Connection(
          config,
          Object.assign({ shouldPlayDisconnect: () => false }, options)
        );
        mediaStream.onclose();
    ...
    describe("pstream.cancel event", () => {
      ...
      context("when the callsid does not match", () => {
        ...
        it("should not emit a cancel event", () => {
          conn.on("cancel", () => {
            throw new Error("Was expecting cancel to not be emitted");
          });
          pstream.emit("cancel", { callsid: "foo" });
        });
      });
    ...
    describe("pstream.ringing event", () => {
      [Connection.State.Connecting, Connection.State.Ringing].forEach(
        (state: Connection.State) => {
          context(
            `when state is ${state} and enableRingingState is false`,
            () => {
              beforeEach(() => {
                (conn as any)["_status"] = state;
              });
              ...
              context("and sdp is present", () => {
                ...
                it("should not emit accept if mediastream is not open", () => {
                  mediaStream.status = "closed";
                  conn.on("accept", () => {
                    throw new Error("Was expecting accept to not be emitted");
                  });
                  pstream.emit("ringing", { sdp: "foo" });
                });
              });
          ...
          context(
            `when state is ${state} and enableRingingState is true`,
            () => {
              beforeEach(() => {
                conn = new Connection(
                  config,
        ...
        for (let i = 1; i <= dataLength; i++) {
          ...
          const sample = {
            ...
            audioInputLevel: Math.round(
              (convert(internalAudioIn) + convert(2)) / 2
            ),
            audioOutputLevel: Math.round(
              (convert(internalAudioOut) + convert(2)) / 2
            ),
            inputVolume: i,
            outputVolume: i,
          };
  ...
  describe("on monitor.warning", () => {
    ...
    context("if warningData.name contains audio", () => {
      ...
      it("should emit a warning event", (done) => {
        const data = {
          name: "audioInputLevel",
          threshold: { name: "maxDuration", value: 1 },
          values: [1, 2, 3],
        };
        mediaStream.isMuted = false;
    ...
    context("if warningData.name does not contain audio", () => {
      ...
      [
        ...
        it(`should emit a warning event for ${item.name}`, (done) => {
          const data = {
            name: item.name,
            threshold: { name: item.threshold, value: 1 },
            values: [1, 2, 3],
          };
          conn.on("warning", (name, warningData) => {
  ...
  describe("on media failed", () => {
    beforeEach(() => {
      ...
      conn = new Connection(
        config,
        Object.assign(options, { enableIceRestart: true })
      );
    ...
    context("on ICE Gathering failures", () => {
      ...
      it("should publish warning on ICE Gathering timeout", () => {
        mediaStream.onicegatheringfailure(
          Connection.IceGatheringFailureReason.Timeout
        );
        sinon.assert.calledWith(
          publisher.warn,
          "ice-gathering-state",
      ...
      it("should publish warning on ICE Gathering none", () => {
        mediaStream.onicegatheringfailure(
          Connection.IceGatheringFailureReason.None
        );
        sinon.assert.calledWith(
          publisher.warn,
          "ice-gathering-state",
