...
describe("Device", function () {
  ...
  const AudioHelper = (
    _updateSinkIds: Function,
    _updateInputStream: Function
  ) => {
    updateInputStream = _updateInputStream;
    updateSinkIds = _updateSinkIds;
    const audioHelper = createEmitterStub(
      require("../../lib/twilio/audiohelper").default
    );
  ...
  const connectionFactory = (
    _?: any,
    _connectOptions?: Record<string, any>
  ) => {
    connectOptions = _connectOptions;
    return (activeConnection = createEmitterStub(
      require("../../lib/twilio/connection").default
    ));
  };
  ...
  const soundFactory = (name: Device.SoundName) =>
    (sounds[name] = sinon.createStubInstance(
      require("../../lib/twilio/sound")
    ));
  const setupOptions = {
    AudioHelper,
    connectionFactory,
  ...
  describe("constructor", () => {
    ...
    it("should throw an error if options are passed but no token", () => {
      assert.throws(
        () => new (Device as any)(undefined, {} as any),
        /Cannot construct a Device/
      );
    });
    ...
    it("should set enableIceRestart to false if passed in as false", () => {
      const conn = device.setup(
        token,
        Object.assign({ enableIceRestart: false }, setupOptions)
      );
      assert.equal(conn["options"].enableIceRestart, false);
    });
    ...
    it("should set enableIceRestart to true if passed in as true", () => {
      const conn = device.setup(
        token,
        Object.assign({ enableIceRestart: true }, setupOptions)
      );
    ...
    it("should set preflight to false if passed in as false", () => {
      const conn = device.setup(
        token,
        Object.assign({ preflight: false }, setupOptions)
      );
      assert.equal(conn["options"].preflight, false);
    });
    ...
    it("should set preflight to true if passed in as true", () => {
      const conn = device.setup(
        token,
        Object.assign({ preflight: true }, setupOptions)
      );
    ...
    it("should set forceAggressiveIceNomination to false if passed in as false", () => {
      const conn = device.setup(
        token,
        Object.assign({ forceAggressiveIceNomination: false }, setupOptions)
      );
      assert.equal(conn["options"].forceAggressiveIceNomination, false);
    });
    ...
    it("should set forceAggressiveIceNomination to true if passed in as true", () => {
      const conn = device.setup(
        token,
        Object.assign({ forceAggressiveIceNomination: true }, setupOptions)
      );
    ...
    describe("log", () => {
      ...
      it("should set log level to DEBUG if debug is true and warnings is true", () => {
        device.setup(
          token,
          Object.assign({ debug: true, warnings: true }, setupOptions)
        );
      ...
      it("should set log level to DEBUG if debug is true and warnings is false", () => {
        device.setup(
          token,
          Object.assign({ debug: true, warnings: false }, setupOptions)
        );
        sinon.assert.calledWith(setDefaultLevelStub, LogLevels.DEBUG);
      });
      ...
      it("should set log level to WARN if debug is false and warnings is true", () => {
        device.setup(
          token,
          Object.assign({ debug: false, warnings: true }, setupOptions)
        );
      ...
      it("should set log level to SILENT if debug is false and warnings is false", () => {
        device.setup(
          token,
          Object.assign({ debug: false, warnings: false }, setupOptions)
        );
  ...
  context("before Device is initialized", () => {
    ...
    describe(".setup()", () => {
      context("when not supported", () => {
        ...
        before(() =>
          Promise.resolve(((window as any).RTCPeerConnection = null))
        );

        it("should throw an exception if not supported", () => {
          assert.throws(
            () => device.setup(token, setupOptions),
            /require WebRTC browser support/
          );
        });
      ...
      context("when legacy edge", () => {
        ...
        before(() =>
          Promise.resolve(
            ((window as any).navigator = { userAgent: "foo edge/123 bar" })
          )
        );
      ...
      context("when supported", () => {
        ...
        it("should throw if both `edge` and `region` are defined in options", () => {
          device = new Device();
          assert.throws(() =>
            device.setup(token, { edge: "foo", region: "bar" })
          );
        });
        ...
        it("should disable the publisher when publishEvents: false is passed", () => {
          ...
          device.setup(
            token,
            Object.assign(setupOptions, { publishEvents: false })
          );
        ...
        it("should set close protection on the page if closeProtection: true", () => {
          ...
          device.setup(
            token,
            Object.assign(setupOptions, { closeProtection: true })
          );
  ...
  context("after Device is initialized", () => {
    ...
    describe(".connect(params?, audioConstraints?, iceServers?)", () => {
      ...
      it("should call ignore on all existing connections", () => {
        ...
        connections.forEach((conn: any) =>
          sinon.assert.calledOnce(conn.incoming)
        );
      ...
      it("should pass rtcConfiguration along", () => {
        assert.deepEqual(
          rtcConfiguration,
          connectOptions && connectOptions.rtcConfiguration
        );
      });
    ...
    describe(".destroy()", () => {
      ...
      it("should disconnect all connections", () => {
        ...
        (device as any)["connections"] = [{ disconnect }, { disconnect }];
        device.destroy();
    ...
    describe(".edge", () => {
      ...
      context("when the region is mapped to a known edge", () => {
        Object.entries(regionShortcodes).forEach(
          ([fullName, region]: [string, string]) => {
    ...
    describe("on signaling.invite", () => {
      ...
      context("if not on an active call and payload is valid", () => {
        ...
        it("should pass the custom parameters to the new connection", () => {
          assert.deepEqual(connectOptions && connectOptions.twimlParams, {
            foo: "bar",
          });
        });
      });
      ...
      context("when allowIncomingWhileBusy is true", () => {
        beforeEach(() => {
          ...
          device.setup(
            token,
            Object.assign(setupOptions, { allowIncomingWhileBusy: true })
          );
          pstream.emit("ready");
          device.connect();
        });
        ...
        context(`when type is 'speaker'`, () => {
          it("should call setSinkIds on all sounds except incoming", () => {
            ...
            Object.values(Device.SoundName)
              .filter(
                (name: Device.SoundName) => name !== Device.SoundName.Incoming
              )
          ...
          context("if unsuccessful", () => {
            ...
            beforeEach(() => {
              device.connect();
              sinkIds = ["default"];
              activeConnection._setSinkIds = sinon.spy(() =>
        ...
        context(`when type is 'ringtone'`, () => {
          it("should call setSinkIds on incoming", () => {
            ...
            sinon.assert.calledOnce(
              sounds[Device.SoundName.Incoming].setSinkIds
            );
            sinon.assert.calledWith(
              sounds[Device.SoundName.Incoming].setSinkIds,
              sinkIds
  ...
  describe("InboundAudioProcessor (Device)", () => {
    it("should store the processor and propagate to new connections", () => {
      const processor = { createProcessedStream: sinon.stub().resolves() };
      device.setup(token, setupOptions);
      device.setInboundAudioProcessor(processor);
      device.connect();
      sinon.assert.calledWith(
        activeConnection.setInboundAudioProcessor,
        processor
      );
    });

    it("should propagate to the active connection if present", () => {
      const processor = { createProcessedStream: sinon.stub().resolves() };
      device.setup(token, setupOptions);
      device.connect();
      device.setInboundAudioProcessor(processor);
...
describe("Device.setInboundAudioProcessor integration", () => {
  let device: Device;
  let processor: any;
  let token: string;
  let activeConnection: any;
  let setupOptions: any;
  let connectionFactory: any;

  beforeEach(() => {
    token = createToken("alice");
    processor = {
      createProcessedStream: sinon.stub().resolves("processed"),
      destroyProcessedStream: sinon.stub().resolves(),
    };
    activeConnection = createEmitterStub(
      require("../../lib/twilio/connection").default
    );
    connectionFactory = sinon.stub().returns(activeConnection);
    setupOptions = {
      connectionFactory,
      AudioHelper: () =>
        createEmitterStub(require("../../lib/twilio/audiohelper").default),
      pStreamFactory: () =>
        createEmitterStub(require("../../lib/twilio/pstream")),
      Publisher: () =>
        createEmitterStub(require("../../lib/twilio/eventpublisher")),
      soundFactory: (name: Device.SoundName) =>
        sinon.createStubInstance(require("../../lib/twilio/sound")),
    };
    device = new Device();
  });

  it("should store the processor on the Device", () => {
    device.setInboundAudioProcessor(processor);
    // @ts-ignore
    assert.equal(device._inboundAudioProcessor, processor);
  });

  it("should propagate to the active connection if present", () => {
    device.setup(token, setupOptions);
    device.connect();
    if (!activeConnection.setInboundAudioProcessor.restore) {
      sinon.spy(activeConnection, "setInboundAudioProcessor");
    }
    device.setInboundAudioProcessor(processor);
    sinon.assert.calledWith(
      activeConnection.setInboundAudioProcessor,
  ...
  it("should use the processor for new connections after being set", () => {
    device.setup(token, setupOptions);
    device.setInboundAudioProcessor(processor);
    device.connect();
    sinon.assert.calledWith(
  ...
  it("should use the processor if set before setup, after setup and connect", () => {
    device.setInboundAudioProcessor(processor);
    device.setup(token, setupOptions);
    device.connect();
    sinon.assert.calledWith(
  ...
  it("should propagate to the active connection if set after connect", () => {
    device.setup(token, setupOptions);
    device.connect();
    if (!activeConnection.setInboundAudioProcessor.restore) {
      sinon.spy(activeConnection, "setInboundAudioProcessor");
